...
const LandingPage: React.FC<LandingPageProps> = ({ onEnter }) => {
  return (
    <div className="min-h-screen bg-black text-slate-100 flex flex-col items-center justify-center p-6 relative overflow-hidden font-sans">
      ...
      <div className="absolute inset-0 bg-[linear-gradient(rgba(255,255,255,0.03)_1px,transparent_1px),linear-gradient(90deg,rgba(255,255,255,0.03)_1px,transparent_1px)] bg-[size:30px_30px] opacity-20 z-0 pointer-events-none"></div>

      <div className="relative z-10 max-w-5xl w-full flex flex-col items-center text-center space-y-12 animate-fade-in">
          ...
          {[
            {
              step: "01",
              title: "Configure",
              desc: "Select your proficiency level (e.g., Snare 1) and the specific method book or rudiment family you wish to review."
            },
            {
              step: "02",
              title: "Draw",
              desc: "Initiate the chance operation mechanism. The system will randomly select a specific etude or rudiment for your barrier."
            },
            {
              step: "03",
              title: "Analyze",
              desc: "Receive instant, AI-generated technical focus points tailored specifically to your drawn selection."
            }
          ].map((item, idx) => (
        ...
        <div className="pt-8">
            ...
            <div className="absolute inset-0 border border-white/20 scale-105 opacity-0 group-hover:opacity-100 group-hover:scale-110 transition-all duration-300"></div>
            </button>
        </div>

      </div>

      <footer className="absolute bottom-6 text-neutral-700 text-[10px] uppercase tracking-widest">
        Official Barrier Review Tool
      </footer>
    </div>
  );
};
